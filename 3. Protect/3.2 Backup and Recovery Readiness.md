...
Note that backups stored on the same network, using the same admin account, with no restore test, is weak.
...
A backup system that fails silently is inadequate, alerts must occur and be seen.

## Step 11: Test Restores

Restore testing is mandatory - a restore test proves that backup data can actually be used.
