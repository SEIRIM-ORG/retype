...
icon: triangle-right
---
The goal of the **Identify** stage is to create a clear, accurate map of the company’s digital environment so the business knows exactly what must be protected, who can access it, where critical data lives, and what systems or vendors the company depends on.

A strong summary:

**Identify turns cybersecurity from guesswork into visibility.**

The objective is not yet to judge risk or design controls. That belongs more to **Assess** and **Protect**. The objective here is to build the factual inventory that every later cybersecurity decision depends on.

The main objectives are:

**1. Know what assets exist.**  
Create an accurate inventory of devices, servers, cloud resources, network equipment, and other technology assets. The company should know what it owns, what it uses, where each asset is located, who is responsible for it, and whether it is still active.

**2. Know what data exists and where it lives.**  
Identify business-critical, sensitive, confidential, regulated, and operational data. The company should understand where data is stored, how it moves, who uses it, and which data would cause serious harm if lost, stolen, leaked, or corrupted.

**3. Know which applications and services support the business.**  
List the software, SaaS tools, platforms, databases, websites, APIs, and cloud services the company relies on. This helps expose shadow IT, forgotten systems, duplicate tools, and business-critical platforms that may not be formally tracked.

**4. Know who and what has access.**  
Identify users, administrators, contractors, service accounts, shared accounts, vendor accounts, API keys, and automation identities. The aim is to understand every access point into the environment, especially privileged or unmanaged access.

**5. Know the dependencies and connections.**  
Map the relationships between systems, users, data, vendors, cloud services, integrations, and business processes. This shows what breaks if a key system, supplier, account, or service becomes unavailable or compromised.

A cleaner executive-level version:
...
For an SME playbook, I would phrase it this way:
...
The blunt version: if this stage is weak, the rest of the playbook is built on fantasy. You cannot secure assets you do not know exist, protect data you have not located, remove access you have not identified, or recover systems whose dependencies you never mapped.
